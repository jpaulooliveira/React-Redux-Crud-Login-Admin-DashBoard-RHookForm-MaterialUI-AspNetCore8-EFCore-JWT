...
import { categoryRepository, useCategory } from "../../../pages-hooks/useCategories";
...
export function SectionCategory() {
  ...
  const { getCategory, editCategory, createCategory } = categoryRepository();
  ...
  const {
    ...
    defaultValues: async () => await getCategory(Number(params.id))
  });

  const onSubmit = async (data: CategoryFormInput) => {
    if (params?.id)
      await editCategory(Number(params?.id), data)
    else
